...
bool pointInsideFOV(const FOV& fov, const PolarPoint& p_pol) {
    // 归一化左右边界和点的方位角到 [-180, 180)
    const float left_bound = wrapAngleToPlusMinus180(fov.yaw_deg - fov.h_fov_deg / 2.f);
    const float right_bound = wrapAngleToPlusMinus180(fov.yaw_deg + fov.h_fov_deg / 2.f);
    const float normalized_z = wrapAngleToPlusMinus180(p_pol.z);

    // 处理视场角跨越 -180°/180° 的情况
    bool in_azimuth = false;
    if (left_bound > right_bound) {
        // 视场角跨越分界点，检查是否在左边界到 180° 或 -180° 到右边界
        in_azimuth = (normalized_z >= left_bound) || (normalized_z <= right_bound);
    } else {
        // 视场角未跨越分界点，直接判断是否在区间内
        in_azimuth = (normalized_z >= left_bound) && (normalized_z <= right_bound);
    }

    // 仰角检查（无需修改）
    const bool in_elevation = (p_pol.e <= fov.pitch_deg + fov.v_fov_deg / 2.f) &&
...
bool histogramIndexYawInsideFOV(const std::vector<FOV>& fov_vec, const int idx, Eigen::Vector3f position,
  ...
  //std::cout << "pol_fcu: " << idx << "," << pol_fcu.z << std::endl;
...
void updateFOVFromMaxima(FOV& fov, const pcl::PointCloud<pcl::PointXYZ>& maxima) {
  ...
  //std::cout << "fov.yaw_deg: " << fov.yaw_deg << "," << "h_fov_deg:" << fov.h_fov_deg << std::endl;
